...
class CellClassifier:
    ...
    def _plot_probability_distribution(self, all_probs: List[float], all_labels: List[int], output_dir: Path):
        ...
        # Plot thresholds
        ax1.axvline(x=self.config.confidence_threshold_low, color='green', linestyle='--', 
        ...
        ax1.axvline(x=self.config.confidence_threshold_high, color='red', linestyle='--', 
    ...
    def _plot_confidence_distribution(self, all_probs: List[float], output_dir: Path):
        ...
        ax1.axvline(x=self.config.confidence_threshold_high, color='red', linestyle='--', 
                   label=f'High Confidence ({self.config.confidence_threshold_high:.2f})')
        ax1.axvline(x=self.config.confidence_threshold_low, color='green', linestyle='--', 
                   label=f'Low Confidence ({self.config.confidence_threshold_low:.2f})')
        ...
        decisions = []
        for p in all_probs:
            if p >= self.confidence_threshold_high:
                decisions.append('Cancerous')
            elif p <= self.confidence_threshold_low:
        ...
        prob_by_decision = {
            'Cancerous': [p for p in all_probs if p >= self.confidence_threshold_high],
            'Non-Cancerous': [p for p in all_probs if p <= self.confidence_threshold_low],
            'Uncertain': [p for p in all_probs if self.confidence_threshold_low < p < self.confidence_threshold_high]
        }
    ...
    def _plot_ternary_probability_distribution(self, all_probs: List[np.ndarray], all_labels: List[int], output_dir: Path):
        ...
        for class_idx in range(3):
            ...
            ax.axvline(x=self.config.confidence_threshold_high, color='red', linestyle='--', 
                      label=f'High Threshold ({self.config.confidence_threshold_high:.2f})')
            ax.axvline(x=self.config.confidence_threshold_low, color='green', linestyle='--', 
                      label=f'Low Threshold ({self.config.confidence_threshold_low:.2f})')
        ...
        ax4.axvline(x=self.uncertainty_threshold, color='red', linestyle='--', 
                   label=f'Uncertainty Threshold ({self.uncertainty_threshold:.2f})')
    ...
    def _plot_confidence_distribution_ternary(self, all_confidences: List[float], output_dir: Path):
        ...
        ax1.axvline(x=self.uncertainty_threshold, color='red', linestyle='--', 
                   label=f'Uncertainty Threshold ({self.uncertainty_threshold:.2f})')
        ...
        decisions = ['Certain' if c >= self.uncertainty_threshold else 'Uncertain' for c in all_confidences]
